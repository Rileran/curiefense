...
impl IntegerMetric {
    ...
    fn to_json(&self) -> Value {
        if self.n_sample == 0 {
            ...
            return serde_json::json!({ "min": f64::NAN, "max": f64::NAN, "average": f64::NAN });
        }
        serde_json::json!({
            "min": self.min,
            "max": self.max,
...
impl AggregatedCounters {
    fn increment(&mut self, dec: &Decision, rcode: Option<u32>, rinfo: &RequestInfo, tags: &Tags) {
        ...
        let processing_time_in_ms = Utc::now().signed_duration_since(rinfo.timestamp).num_milliseconds();
        self.processing_time.increment(processing_time_in_ms);
